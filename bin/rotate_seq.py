...
def rotate_seq(rec, positions, anchor_pos, rcomp, motif):
    """
    Rotates a sequence record using a list of possible positions to rotate to and an anchor sequence to get close to
    ...
    return:
        the rotated sequence record object
    """
    ...
    if len(positions) != 0:
        ...
        if rcomp:
            shift_offset = (shift_offset + len(motif)) % len(rec)
...
def get_names(rec_list):
    ...
    """
    return ", ".join([r.id for r in rec_list])
